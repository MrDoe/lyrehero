...
export const TutorInterface: React.FC<TutorInterfaceProps> = ({
  ...
  const handleCorrectNote = useCallback(() => {
    ...
    if (
      nextIndex < song.notes.length &&
      song.notes[nextIndex].note === completedNote
    ) {
  ...
  const checkPitch = useCallback(() => {
    ...
    if (result) {
      ...
      if (Math.random() < 0.033) {
        console.log("UI checkPitch:", {
          rms: result.volume.toFixed(4),
  ...
  const stopNoteCalibration = () => {
    ...
    if (detectedNote && detectedNote !== "..." && currentClarity > 0.01) {
      ...
      const newClarityThreshold = Math.max(
        0.005,
        Math.min(currentClarity * 0.5, 0.3)
      );
        ...
        {isFinished ? (
          <div className="flex-1 flex flex-col items-center justify-center p-8">
            <div className="text-center animate-in fade-in zoom-in duration-500 p-8 bg-slate-800/50 rounded-3xl border border-slate-700 shadow-2xl max-w-md w-full">
              ...
              <h3 className="text-3xl font-bold mb-4 bg-clip-text text-transparent bg-gradient-to-r from-green-400 to-emerald-600">
                Song Complete!
              </h3>
              <p className="text-slate-300 mb-8 text-lg">
                Great job playing {song.title}
              </p>
              <button
                      ...
                      <div className="grid grid-cols-2 gap-4 mb-6 bg-slate-900/50 p-4 rounded-xl border border-slate-700/50">
                        ...
                        <div>
                          <div className="flex justify-between text-xs text-slate-400 mb-1">
                            ...
                            <span
                              className={
                                currentRms >= config.rmsThreshold
                          ...
                          <div className="h-2 bg-slate-700 rounded-full overflow-hidden">
                            <div
                              ...
                              style={{
                                width: `${Math.min(currentRms * 1000, 100)}%`,
                          ...
                          <div className="text-[10px] text-slate-600 mt-1 flex justify-between">
                            <span>
                              Threshold:{" "}
                              {(config.rmsThreshold * 1000).toFixed(1)}
                            </span>
                          </div>
                        </div>
                        ...
                        <div>
                          <div className="flex justify-between text-xs text-slate-400 mb-1">
                            ...
                            <span
                              className={
                                currentClarity >= config.correlationThreshold
                                  ? "text-green-400 font-bold"
                                  : "text-slate-500"
                              }
                            >
                      ...
                      <div className="bg-slate-900/70 p-4 rounded-xl border border-slate-700/50">
                          ...
                          <div className="flex items-center justify-between mb-2">
                            ...
                            {noiseFloor > 0 && (
                              <span className="text-xs text-green-400">
                                ✓ Done ({(noiseFloor * 1000).toFixed(2)})
                              </span>
                            )}
                          </div>
                        ...
                        <div>
                          ...
                          <p className="text-[10px] text-slate-600 mt-1">
                            Increase if notes aren't detected. Decrease if
                            signal clips.
                          </p>
                        </div>
                      </div>
                      ...
                      <div>
                        <div className="flex justify-between text-sm mb-2">
                          ...
                          <span className="font-mono text-indigo-400">
                            {config.rmsThreshold.toFixed(4)}
                          </span>
                        </div>
                        <label htmlFor="rmsThreshold" className="sr-only">
                          Volume Threshold
                        </label>
                        <input
                          id="rmsThreshold"
              ...
              <div className="max-w-xl mx-auto flex flex-col gap-3 sm:gap-6">
                  ...
                  <div className="flex-shrink-0">
                    ...
                    <div
                      className={`text-2xl sm:text-3xl font-mono font-bold ${
                        detectedNote === song.notes[currentIndex]?.note
                ...
                <div className="flex justify-center">
                  <button
                    ...
                    className={`flex items-center gap-2 text-xs uppercase font-bold tracking-wider px-4 py-2 rounded-full transition-all ${
                      showSettings
                  ...
                  >
                    <Settings2 size={14} />
                    <span>
                      {showSettings ? "Hide Configuration" : "Configuration"}
                    </span>
                  </button>
                </div>
